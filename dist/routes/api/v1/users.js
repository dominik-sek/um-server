...
router.get('/', (0, authPage_1.authRole)(userRole_1.UserRole.ADMIN), async (req, res) => {
    try {
        const result = await prisma_1.default.person.findMany({
            include: {
                address: true,
                contact: true,
                personal: true,
...
router.get('/profile', (0, authPage_1.authRoleOrPerson)([userRole_1.UserRole.ADMIN, userRole_1.UserRole.TEACHER, userRole_1.UserRole.STUDENT]), async (req, res) => {
...
router.get('/:id', (0, authPage_1.authRoleOrPerson)(userRole_1.UserRole.ADMIN), async (req, res) => {
    try {
        const result = await prisma_1.default.person.findUnique({
            where: {
...
router.post('/', (0, authPage_1.authRole)(userRole_1.UserRole.ADMIN), async (req, res) => {
    try {
        const newPerson = await prisma_1.default.person.create({
            data: {
        ...
        if (req.body.role === userRole_1.UserRole.STUDENT) {
            await prisma_1.default.department_students.create({
                data: {
            ...
            if (courses) {
                for (let i = 0; i < courses.length; i++) {
                    await prisma_1.default.course_students.create({
                        data: {
...
router.put('/profile/avatar', (0, authPage_1.authRoleOrPerson)([userRole_1.UserRole.ADMIN, userRole_1.UserRole.TEACHER, userRole_1.UserRole.STUDENT]), async (req, res) => {
    try {
        const result = await prisma_1.default.account.update({
            where: {
...
router.put('/profile/background', (0, authPage_1.authRoleOrPerson)([userRole_1.UserRole.ADMIN, userRole_1.UserRole.TEACHER, userRole_1.UserRole.STUDENT]), async (req, res) => {
    try {
        const result = await prisma_1.default.account.update({
            where: {
                person_id: req.user?.person_id
...
router.put('/:id', (0, authPage_1.authRoleOrPerson)(userRole_1.UserRole.ADMIN), async (req, res) => {
    try {
        ...
        const result = await prisma_1.default.person.update({
            ...
            data: {
                ...
                birth_date: (0, moment_1.default)(req.body.birth_date, 'DD/MM/YYYY').toDate() || undefined,
...
router.delete('/:id', (0, authPage_1.authRole)(userRole_1.UserRole.ADMIN), async (req, res) => {
    try {
        const result = await prisma_1.default.person.delete({
            where: {
                id: Number(req.params.id),
            },
        });
